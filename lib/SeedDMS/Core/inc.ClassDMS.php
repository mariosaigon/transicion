...
class SeedDMS_Core_DMS {
	...
	{ /* {{{ */
		...
		//echo "CLASS DMS: imprimiendo argumentos";
		//echo "fecha INICIAL:  "; //genera algo como Array ( [year] => 2017 [month] => 8 [day] => 8 [hour] => 0 [minute] => 0 [second] => 0 )
		//print_r($fecha_inicial);
		//echo "fecha FINAL:  "; //genera algo como Array ( [year] => 2017 [month] => 8 [day] => 8 [hour] => 0 [minute] => 0 [second] => 0 )
		//print_r($fecha_final);
		//echo "booleana quiero rango: ".$quiero_buscar_rango; //COMPROBARÉ SI ES TRUE CON = = 1 o TRUE
		...
		if($mode & 0x2) {
			...
			//ESTO ES LO QUE PUEDO RECICLAR PARA RANGO DE FECHAS
			$searchCreateDate = "";
			if ($creationstartdate) 
			{
				//hace un timestamp con la fecha de "inicio" del rango para poder compararla.
			...
			/*
			$queryFechaInicial= "";
			if($fecha_inicial) //comprueba si no está vacío el atributo fecha inicial
			{
				$marca_fecha_inicial=SeedDMS_Core_DMS::makeTimeStamp($fecha_inicial['hour'], $fecha_inicial['minute'], $fecha_inicial['second'], $fecha_inicial['year'], $fecha_inicial["month"], $fecha_inicial["day"]);
				//echo "MARCA fecha inicial:::::: ".$marca_fecha_inicial;
				if ($marca_fecha_inicial) 
				{
					//echo "RESULTADO POLAAAA";
					//str_to_date(date_field, '%d-%m-%Y') BETWEEN '2010-01-30' AND '2010-09-29'
					//$queryFechaInicial = "EXISTS (SELECT NULL FROM `tblDocumentAttributes` WHERE `tblDocumentAttributes`.`attrdef`= 5 AND (str_to_date(`tblDocumentAttributes`.`value` , '%Y-%m-%d' ) BETWEEN '2010-01-30' AND '2019-09-29') AND `tblDocumentAttributes`.`document` = `tblDocuments`.`id`)";
					$queryFechaInicial="EXISTS (SELECT NULL FROM `tblDocumentAttributes` WHERE `tblDocumentAttributes`.`attrdef`= 5 AND strftime (`tblDocumentAttributes`.`value` ) between strftime('2013-01-01') and strftime('2019-01-01') AND `tblDocumentAttributes`.`document` = `tblDocuments`.`id`)";
			...
			*/
			...
			// if (strlen($queryFechaInicial)>0) //comprueba que la longitud de la query es mayor que cero (es decir, tiene contenido), 
			// {
				
				// $searchQuery .= " AND (".$queryFechaInicial.")";
			// }
		...
		if($mode & 0x1) {
			...
			$hare_rango=FALSE;
			////////----------------------------------------------------------------------------------///////////////
			if($quiero_buscar_rango==1)
			{
			//echo "/*/*/* ELEGIDA FECHA INICIAL */*/*";
			$year_inicial=$fecha_inicial['year'];
			$mes_inicial=$fecha_inicial['month'];
			if($mes_inicial>=1 && $mes_inicial<=9) //hago la comprobación pues el método strftime funciona con el formato (2010-01-01) es decir, con un cero antes de los números 1-9; el formulario solo me devuelve los números del 1-9 sin el cero, por eso debo agregarlos.
			{
				$mes_inicial="0".$mes_inicial;
			}
			$dia_inicial=$fecha_inicial['day'];
			if($dia_inicial>=1 && $dia_inicial<=9) //hago la comprobación pues el método strftime funciona con el formato (2010-01-01) es decir, con un cero antes de los números 1-9; el formulario solo me devuelve los números del 1-9 sin el cero, por eso debo agregarlos.
			{
				$dia_inicial="0".$dia_inicial;
			}
			$fecha_inicial_formateada=$year_inicial."-".$mes_inicial."-".$dia_inicial;
			//mismo proceso para la fecha final
			$year_final=$fecha_final['year'];
			$mes_final=$fecha_final['month'];
			if($mes_final>=1 && $mes_final<=9) //hago la comprobación pues el método strftime funciona con el formato (2010-01-01) es decir, con un cero antes de los números 1-9; el formulario solo me devuelve los números del 1-9 sin el cero, por eso debo agregarlos.
			{
				$mes_final="0".$mes_final;
			}
			$dia_final=$fecha_final['day'];
			if($dia_final>=1 && $dia_final<=9) //hago la comprobación pues el método strftime funciona con el formato (2010-01-01) es decir, con un cero antes de los números 1-9; el formulario solo me devuelve los números del 1-9 sin el cero, por eso debo agregarlos.
			{
				$dia_final="0".$dia_final;
			}
			$fecha_final_formateada=$year_final."-".$mes_final."-".$dia_final;
			// se conforma el array de bósqueda con las fechas formateadas (con los ceros) 
			//EN MI LOCAL EL ATRIBUTO FECHA DE CLASISIFACION ES ID 2 EN LA BD
			//echo "fecha inicial: ".$fecha_final_formateada;
			$searchAttributes[] = "EXISTS (SELECT NULL FROM `tblDocumentAttributes` WHERE `tblDocumentAttributes`.`attrdef`= 2 AND (`tblDocumentAttributes`.`value` ) between '".$fecha_inicial_formateada."' and '".$fecha_final_formateada."' AND `tblDocumentAttributes`.`document` = `tblDocuments`.`id`)";
			
			}
			////////----------------------------------------------------------------------------------///////////////
			if ($attributes) 
			{
				foreach($attributes as $attrdefid=>$attribute) //Bucle que recorre cada atributo personalizado
				{
					if($attribute)
						{
						$attrdef = $this->getAttributeDefinition($attrdefid);
						if($attrdef->getObjType() == SeedDMS_Core_AttributeDefinition::objtype_document || $attrdef->getObjType() == SeedDMS_Core_AttributeDefinition::objtype_all) // atributos de tipo "TODO"
						{
							/**
							IMPORTANTE: EL ID DEL ATRIBUTO FECHA ES EL 5
							**/
							$nomAtributo=$attrdef->getName();
							$id_atributo=$attrdef->getID();
							
						    //echo "nombre de atributo analizado: ".$nomAtributo.PHP_EOL;
							  //echo "ID de atributo analizado: ".$id_atributo.PHP_EOL;
							if($valueset = $attrdef->getValueSet()) //SE mete aquí si el atributo en cuestión, tiene varios "campos" (posibles valores, por ejemplo, a,b,c)
							{
								//echo "-------UNO---------";
								
								if($attrdef->getMultipleValues()) //aqui obtengo todos los posibles valores
								{
									//echo "-------DOS---------";
							...
							else // se mete aqui si el atributo tiene un único valor
							{
								
								//echo "-------CUATRO---------";
								//AQUI DEBO FILTRAR POR FECHAS PARA EL CASO DE FECHA, ID=5
								if($fecha_inicial)
								{
						...
						elseif($attrdef->getObjType() == SeedDMS_Core_AttributeDefinition::objtype_documentcontent) //atributos de tipo DOCUMENT, no debería tocarlo
						{
		...
		if($limit) {
         ...
         echo "core:     ".$searchQuery;
		 //$fich_output="C:\xampp\htdocs\query.txt";
		// $fich_output = fopen("C:/xampp/htdocs/query.txt", "r+");
		 file_put_contents("pene.txt", $searchQuery);
