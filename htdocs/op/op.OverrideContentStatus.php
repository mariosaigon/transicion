...
function insertarEnBD($documento,$fecha) //dada una fecha, mete en la tabla revocacionReservas la fecha con el documento
{
	...
	if($estado!=1)
	{
		UI::exitError(getMLText("document_title", array("documentname" => getMLText("invalid_doc_id"))),getMLText("op.OverrideContentStatus: No se pudo conectar a la BD"));
	}	
	//query de consulta:
	$miQuery="INSERT INTO revocacionReservas (idDocumento,fechaResolucion) values('$documento','$fecha')";
	//echo "mi query: ".$miQuery;
	$resultado=$manejador->getResult($miQuery);
	//echo "fecha= ".$fecha;
    if(!$resultado)
	{
		UI::exitError(getMLText("document_title", array("documentname" => getMLText("invalid_doc_id"))),getMLText("Parece ser que la fecha introducida es incorrecta. Inténtelo nuevamente."));
	}
...
//agreado por Mario  15 oct
if (!isset($_POST["fechaRevocacion"]))
{
	UI::exitError(getMLText("document_title", array("documentname" => $document->getName())),getMLText("Fecha de revocación de reserva incorrecta."));
}
...
$fechaRevocacion = $_POST["fechaRevocacion"];
insertarEnBD($documentid,$fechaRevocacion);
//añadido por Mario: agrego a la tabla de la BD revocacionReservas la fecha de la revocación
