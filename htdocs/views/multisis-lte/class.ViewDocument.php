...
<?php
	...
	function show() { /* {{{ */
		...
		if($user->isAdmin())
		{
			$this->htmlStartPage("ver documento ".$document->getName(), "skin-blue sidebar-mini sidebar-collapse");
		}
		else
		{
			$this->htmlStartPage("ver documento ".$document->getName(), "skin-blue layout-top-nav");
		}
		$this->containerStart();
		$this->mainHeader();
		if($user->isAdmin())
    ...
    print "</td>";
      ...
      print "</ul></td>\n";
		...
		if($accessop->mayOverwriteStatus()) 
		{
			...
			if($status["status"] != -2)
			{
				print "<a type=\"button\" class=\"btn btn-warning btn-action\" href='../out/out.OverrideContentStatus.php?documentid=".$documentid."&version=".$latestContent->getVersion()."' data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"".getMLText("change_status")."\"><i class=\"fa fa-balance-scale \"></i></a>";
			}
		}
		...
		{
			//eliminado por Mario: no se necesita lo de "Establecer flujo de trabajo"
		...
		if ($theaccessMode3 >= M_READWRITE || ($document->getAccessMode($user) >= M_READWRITE)) {
			if (!$this->params['user']->isGuest())  //Modificado por Mario, original: !$this->params['user']->isGuest()
			{
				if($status["status"] != -2  ) //modificado por Mario
				//añadido || !$status["status"] == S_RELEASED
				{
